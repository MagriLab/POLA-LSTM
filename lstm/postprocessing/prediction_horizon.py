...
def compute_pred_horizon_idx(pred, num_sol, threshold=0.2, window_size=100):
    ...
    nominator = index_l2_norm(num_sol[window_size: window_size + len(pred)] - pred)
    denominator = index_l2_norm(num_sol[window_size: window_size + len(pred)])

    pred_idx = len(pred)
    for i in range(1, len(pred)):
        if nominator[i] > 0.2*denominator[i]:
            return i-1

    return pred_idx

def predict_horizon_layapunov_time(pred, num_sol, time_vector, window_size=100):
    pred_idx = compute_pred_horizon_idx(pred, num_sol, window_size=100)
