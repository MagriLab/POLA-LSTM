...
def main():
    def run_lstm():
        ...
        def pi_loss(model_prediction, delta_t, norm_val=1):
            mse = tf.keras.losses.MeanSquaredError()
            missing_idx = list(set(range(0, 3)))  # .difference(idx_lst))
            pi_loss = mse(tf.gather(backward_diff(model_prediction), missing_idx, axis=2),
                          tf.gather(l63_batch(model_prediction)[:, :-1, :], missing_idx, axis=2))
            return pi_loss
        ...
        def train_step_pi(model, x_batch_train, y_batch_train, weight_reg=1, weight_pi=0.0):
            with tf.GradientTape() as tape:
                ...
                loss_pi = pi_loss(one_step_pred, 0.01, norm_val=norm_val_arr)
        ...
        def valid_step_pi(model, x_batch_valid, y_batch_valid):
            ...
            loss_pi = pi_loss(val_logit, 0.01, norm_val=norm_val_arr)
            return loss_dd, loss_reg, loss_pi
        ...
        idx_lst = random.sample(range(1, 3+1), args.n_random_idx)
        idx_lst.sort()
        print(idx_lst)
        ...
        for epoch in range(1, args.n_epochs+1):
            ...
            for val_step, (x_batch_valid, y_batch_valid) in enumerate(valid_dataset):
                val_loss_dd, val_loss_reg, val_loss_pi = valid_step_pi(model, x_batch_valid, y_batch_valid)
    ...
    parser.add_argument('--lyap', type=float, default=0.9)
...
# python sweep_l63_pi.py  -cp Yael_CSV/L63l63_euler_57500_0.01_stand21.29_29.01_53.75_trans.csv -dp l63-pi/ -lyp Yael_CSV/L63/l63_lyapunov_exponents.txt
