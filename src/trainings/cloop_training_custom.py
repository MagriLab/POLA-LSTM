...
sys.path.append('../../')
from lstm.loss import lorenz, loss_oloop, norm_pi_loss, norm_pi_loss_two_step
...
def append_label(batch, label):
    return tf.concat((batch, einops.rearrange(label, "i j -> i 1 j")), axis=1)


def split_batch_label(batch_label_tensor, window_size=100, batch_size=32):
    label = batch_label_tensor[:, -1, :]
    batch = batch_label_tensor[:, -(window_size+1):-1, :]
    return batch, label

def decayed_learning_rate(step, init_learning_rate=0.001):
...
def run_lstm(args: argparse.Namespace):
    ...
    tf.function
    def eval_step(x_batch_train, y_batch_train):
        one_step_pred = model(x_batch_train, training=True)
        mse = tf.keras.losses.MeanSquaredError()
        loss_val = mse(y_batch_train, one_step_pred)
        print("Loss of 1st Iteration %.3E"%loss_val)

    @tf.function
    def train_step_pi(x_batch_train, y_batch_train, weight=1, normalised=True):
        with tf.GradientTape() as tape:
            one_step_pred = model(x_batch_train, training=True)
            new_batch, new_label = split_batch_label(append_label(x_batch_train, one_step_pred))
    ...
    def valid_step_pi(x_batch_valid, y_batch_valid):
        val_logit_one_step = model(x_batch_valid, training=False)
        new_batch, new_label = split_batch_label(append_label(x_batch_valid, val_logit_one_step))
        val_logit_two_step = model(new_batch, training=False)
        loss_dd = loss_oloop(y_batch_valid, val_logit_one_step)
        loss_pi = norm_pi_loss_two_step(val_logit_one_step, val_logit_two_step)
        return loss_dd, loss_pi
    ...
    for epoch in range(args.n_epochs+1):
        ...
        for step, (x_batch_train, y_batch_train) in enumerate(train_dataset):
            # eval_step(x_batch_train, y_batch_train)
...
parser.add_argument('--physics_weighing', type=float, default=0.1)
...
# python cloop_training_custom.py -dp ../models/euler/10000/pi-lstm/ -cp ../lorenz_data/CSV/10000/euler_10000_norm.csv
# -idp /Users/eo821/Documents/PhD_Research/PI-LSTM/Lorenz_LSTM/src/models/10000/model/10000/weights
