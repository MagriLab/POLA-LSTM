...
for i in range(1, window_size+1):
    u_t = test_window[:, i-1, :]
    u_t, h, c = lstm_step_comb(u_t, h, c, model, i, 2)
    pred[i, :] = u_t
    
i=window_size
jacobian, u_t, h, c = step_and_jac(u_t, h, c, model, i, 2)
pred[i, :] = u_t
...
for i in range(window_size+1, Ntransient):
    jacobian, u_t, h, c = step_and_jac_analytical(u_t, h, c, model, i, 2)
    pred[i, :] = u_t
    delta = np.matmul(jacobian, delta)
