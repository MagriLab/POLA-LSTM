...
if gpus:
    ...
    try:
     ...
     # Virtual devices must be set before GPUs have been initialize
        print(e)
sys.path.append('../..')
from lstm.lorenz96 import l96_batch, backward_diff
...
def main():
    def run_lstm():
        ...
        def pi_loss(model_prediction, delta_t, stand_val=1):
            mse = tf.keras.losses.MeanSquaredError()
            missing_idx = list(set(range(0, 10)))  # .difference(idx_lst))
            pi_loss = mse(tf.gather(backward_diff(model_prediction*stand_val), missing_idx, axis=2),
                          tf.gather(l96_batch(model_prediction*stand_val, p=8)[:, :-1, :], missing_idx, axis=2))
            return pi_loss
        ...
        def train_step_pi(model, x_batch_train, y_batch_train, weight_reg=1, weight_pi=0.0):
            with tf.GradientTape() as tape:
                ...
                loss_pi = pi_loss(one_step_pred, 0.01, stand_val=13.33)
        ...
        def valid_step_pi(model, x_batch_valid, y_batch_valid):
            ...
            loss_pi = pi_loss(val_logit, 0.01, stand_val=13.33)
            return loss_dd, loss_reg, loss_pi
        ...
        filepath = args.data_path / f"D10_pi-{args.n_random_idx}" / str(wand.name)
        reset_random_seeds()
